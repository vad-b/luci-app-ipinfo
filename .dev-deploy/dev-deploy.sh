...
confirm_generate_key() {
  ...
  if ! is_tty; then
    die "SSH key is missing: $path. Generate it manually: ssh-keygen -t ed25519 -N '' -f '$path' -C 'luci-app-ipinfo-dev-deploy'"
  fi
...
ensure_local_key() {
  ...
  if [[ ! -f "$key_path" ]]; then
    ...
    ssh-keygen -t ed25519 -N "" -f "$key_path" -C "luci-app-ipinfo-dev-deploy" >/dev/null
...
runtime_key_path() {
  ...
  local runtime_key="$runtime_dir/luci-app-ipinfo-dev-deploy-${$}.key"
  mkdir -p "$runtime_dir"
...
load_mapping() {
  ...
  local line_no=0 line src dst mode policy src_abs
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    IFS=$'\t' read -r src dst mode policy _extra <<<"$line"
    src="$(trim "${src:-}")"
    dst="$(trim "${dst:-}")"
    mode="$(trim "${mode:-}")"
    policy="$(trim "${policy:-overwrite}")"
    ...
    [[ "$policy" == "overwrite" || "$policy" == "keep-if-exists" ]] || die "Invalid policy at line $line_no: $policy"
...
warn_git_exec_bits() {
  ...
  for ((i=0; i<${#MAP_SRC[@]}; i++)); do
    ...
    if [[ -z "$git_mode" ]]; then
      log "WARN mapped executable is not tracked by git: $repo_rel"
      continue
    fi
    if [[ "$git_mode" != "100755" ]]; then
      log "WARN git mode for mapped executable is $git_mode, expected 100755: $repo_rel"
    fi
  done
}
...
copy_files() {
  ...
  for ((i=0; i<${#MAP_SRC[@]}; i++)); do
    ...
    if [[ "$policy" == "keep-if-exists" ]] && remote "test -e $(remote_quote "$dst")"; then
      log "KEEP_EXISTS ${MAP_SRC[$i]} -> $dst"
      continue
    fi

    if [[ "$policy" == "keep-if-exists" ]]; then
      log "COPY_MISSING ${MAP_SRC[$i]} -> $dst"
    else
...
set_perms() {
  step "Permissions: set modes for existing mapped files on target"
...
clean_target_files() {
  step "Clean: remove overwrite files on target"
  local i dst policy
  for ((i=0; i<${#MAP_DST[@]}; i++)); do
    dst="${MAP_DST[$i]}"
    policy="${MAP_POLICY[$i]}"
    if [[ "$policy" == "keep-if-exists" ]]; then
      log "KEEP_CLEAN_SKIP $dst"
      continue
    fi
    log "REMOVE $dst"
...
main() {
  ...
  if [[ $# -gt 1 ]]; then
    die "Too many arguments. Use: .dev-deploy/dev-deploy.sh [deploy|clean]"
  fi
  if [[ $# -eq 1 ]]; then
    case "$1" in
      -h|--help)
